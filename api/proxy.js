...
const PROFILE_HTML_FILE = 'index.html';

const PROFILE_CACHE_CONTROL = 'public, max-age=60, s-maxage=300';
const CATALOG_CACHE_CONTROL = 'public, max-age=60, s-maxage=300';

const PROFILE_ERROR_MESSAGES = {
    FILE_NOT_FOUND: 'Profile page not found',
    INTERNAL_ERROR: 'Internal server error'
};
...
/**
 * Сервис для управления страницами профилей и каталога
 */
class ProfilePageService {
    ...
    buildHtmlPath(fileName) {
        return path.join(PUBLIC_DIR, PROFILE_DIR, fileName);
    }
    ...
    async readHtmlFile(filePath) {
        try {
            ...
            if (err.code === 'ENOENT') {
                throw new Error(PROFILE_ERROR_MESSAGES.FILE_NOT_FOUND);
            }
            throw err;
        }
    }
     ...
     */
    setPageHeaders(res, cacheControl = PROFILE_CACHE_CONTROL) {
        res.setHeader('Content-Type', 'text/html; charset=utf-8');
        res.setHeader('Cache-Control', cacheControl);
    }

    /**
     * Отдача HTML-страницы профиля
     * @param {Response} res - Express response объект
     */
    async serveProfilePage(res) {
        const htmlPath = this.buildHtmlPath(PROFILE_HTML_FILE);
        const html = await this.readHtmlFile(htmlPath);
    ...
    /**
     * Проверка, является ли путь зарезервированным
     * @param {string} pathSegment - Сегмент пути
     * @returns {boolean} - true если путь зарезервирован
     */
    isReservedPath(pathSegment) {
        return PROFILE_RESERVED_PATHS.includes(pathSegment);
    }

    /**
     * Извлечение username из пути /profile/@username
     * @param {string} fullPath - Полный путь запроса
     * @returns {string|null} - Username или null
     */
    extractUsername(fullPath) {
        const match = fullPath.match(/^\/profile\/@([^\/]+)/);
...
/**
 * GET /catalog — каталог всех пользователей (публичный)
 */
app.get('/catalog', async (req, res) => {
    try {
        await profilePageService.serveProfilePage(res);
    } catch (err) {
        handleProfilePageError(err, res);
    }
});

/**
 * GET /catalog/search — поиск пользователей (публичный)
 */
...
/**
 * GET /profile — свой профиль (требует авторизации)
 */
...
/**
 * GET /profile/me — алиас для своего профиля
 */
...
/**
 * GET /profile/settings — настройки профиля
 */
...
/**
 * GET /profile/favorites — избранное
 */
...
/**
 * GET /profile/activity — активность
 */
...
/**
 * GET /profile/notifications — уведомления
 */
app.get('/profile/notifications', async (req, res) => {
    try {
        await profilePageService.serveProfilePage(res);
    } catch (err) {
        handleProfilePageError(err, res);
...
/**
 * GET /profile/blocked — заблокированные
 */
app.get('/profile/blocked', async (req, res) => {
    try {
        await profilePageService.serveProfilePage(res);
    } catch (err) {
        handleProfilePageError(err, res);
...
/**
 * GET /profile/export — экспорт данных
 */
app.get('/profile/export', async (req, res) => {
    try {
        await profilePageService.serveProfilePage(res);
    } catch (err) {
        handleProfilePageError(err, res);
...
/**
 * GET /profile/qr — QR-код профиля
 */
app.get('/profile/qr', async (req, res) => {
    try {
        await profilePageService.serveProfilePage(res);
    } catch (err) {
        handleProfilePageError(err, res);
...
app.get('/profile/@:username', async (req, res) => {
    try {
        ...
        if (profilePageService.isReservedPath(username)) {
...
/**
 * Обработчик ошибок для Profile routes
 * @param {Error} err - Объект ошибки
 * @param {Response} res - Express response объект
 */
const handleProfilePageError = (err, res) => {
    console.error('[profile/page]', err);
    
    if (err.message === PROFILE_ERROR_MESSAGES.FILE_NOT_FOUND) {
        return res.status(404).send(PROFILE_ERROR_MESSAGES.FILE_NOT_FOUND);
    }
    
    res.status(500).send(PROFILE_ERROR_MESSAGES.INTERNAL_ERROR);
};
